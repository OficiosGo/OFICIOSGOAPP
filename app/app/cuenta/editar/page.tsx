...
export default function EditarPerfilPage() {
  ...
  const [profileImageUrl, setProfileImageUrl] = useState<string | null>(null); // FIX: estado para la URL de la foto
  ...
  useEffect(() => {
    fetch("/api/auth/me").then(r => r.json()).then(async (data) => {
      ...
      if (pData.data) {
        const p = pData.data;
        setProfile(p);
        setProfileImageUrl(p.profileImage || null); // FIX: cargar URL actual de la foto
        setForm({
          headline: p.headline || "",
          bio: p.bio || "",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // FIX: incluir la URL de la foto si existe
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="w-8 h-8 border-3 border-[#F8C927] border-t-transparent rounded-full animate-spin" />
    </div>
  );
    ...
    <>
      ...
      <div className="px-4 py-5 pb-24">
        <div className="flex justify-center mb-6">
          {/* FIX: conectar onUploadComplete para recibir la URL */}
          <ProfilePhotoUpload
        ...
        {success && (
          <div className="p-3 rounded-xl bg-green-50 text-green-600 text-sm font-medium border border-green-100 mb-4">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Electricista matriculado - 12 años de experiencia"
          ...
          <div>
            ...
            <textarea
              ...
              onChange={set("bio")}
              rows={6}
              placeholder={"Listá tus servicios, uno por línea:\n• Instalaciones eléctricas\n• Reparaciones\n• Mantenimiento"}
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="w-full py-4 rounded-2xl bg-[#F8C927] text-[#1A1D2E] font-extrabold text-[15px] disabled:opacity-60 mt-2 active:scale-[0.98] transition-transform"
          >
