...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!profile) return { title: "No encontrado | OficiosGo" };
  ...
  return {
    ...
    openGraph: {
      ...
      description: `★ ${rating} · ${reviewCount} opiniones · ${city}`,
...
function Stars({ rating, count }: { rating: number; count: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5" aria-label={`${rating.toFixed(1)} de 5 estrellas, ${count} opiniones`}>
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((s) => (
          <svg key={s} width="14" height="14" viewBox="0 0 12 12" fill={s <= rounded ? "#F8C927" : "#E5E7EB"} aria-hidden="true">
...
export default async function ProfilePage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    description: profile.headline || profile.bio || `${profile.category.name} en ${profile.city ?? "Villa María"}`,
    address: {
      "@type": "PostalAddress",
      addressLocality: profile.city ?? "Villa María",
    ...
    aggregateRating:
      profile.totalReviews > 0
        ...
        : undefined,
    image: profile.photos[0]?.url || profileImg || undefined,
    telephone: profile.user.phone ? `+54${profile.user.phone.replace(/\D/g, "")}` : undefined,
  };
    ...
    <div className="min-h-screen bg-[#F7F8FA] pb-28">
      ...
      <div className="relative h-[240px] bg-[#1A1D2E]">
        {profile.photos[0] ? (
          <img
            ...
            alt={`Trabajos de ${profile.user.name}`}
          ...
          <div className="w-full h-full bg-gradient-to-br from-[#1A1D2E] via-[#252839] to-[#1A1D2E]">
            <div
              ...
              style={{
                backgroundImage:
                  "radial-gradient(circle at 30% 50%, #F8C927 0%, transparent 60%)",
              }}
            />
          </div>
        )}
      ...
      <div className="mx-4 -mt-14 relative z-10">
        <div
          className={`rounded-[22px] bg-white p-5 shadow-xl ${
            isPremium ? "border-2 border-[#F8C927]" : "border border-gray-100"
          }`}
        >
      ...
      {profile.bio && (
        <section className="mx-4 mt-3 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm">
          <h2 className="text-[14px] font-extrabold text-[#1A1D2E] mb-2">
            Sobre {firstName}
          </h2>
