...
const getCachedFeatured = unstable_cache(
  async () => searchService.getFeatured(8),
  ["home-featured"],
  { revalidate: 120, tags: ["featured"] }
);

const getCachedSponsors = unstable_cache(
  async () => sponsorRepository.getActive("Villa María"),
  ["home-sponsors-villa-maria"],
  { revalidate: 600, tags: ["sponsors"] }
);

const getCachedTotal = unstable_cache(
  async () => professionalRepository.countAll(),
  ["home-total-count"],
  { revalidate: 300, tags: ["total"] }
);
...
export default async function HomePage() {
  ...
  const recentProfessionals = await professionalRepository.getRecent(
    8,
    featured.map((f) => f.id)
  );
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{
            background: "radial-gradient(circle, rgba(248,201,39,0.12), transparent 70%)",
            filter: "blur(40px)",
          }}
        />
      ...
      <section className="mt-5 px-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-[17px] font-black text-[#0F1120]">
      ...
      <section className="px-5 pt-6 pb-28">
        {sponsors.length > 0 && (
          <>
            <h2 className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400 text-center mb-3">
              Negocios que confían en OficiosGo!
            </h2>
            <div className="flex flex-col gap-3 mb-5">
              {sponsors.map((s) => {
                const isPrem = s.tier === "PREMIUM";
                const sponsorWhatsapp = (s as any).whatsapp;
                const sponsorLogo = (s as any).logoUrl;
                return (
                  <div
                    key={s.id}
                    className={`relative overflow-hidden rounded-2xl ${
                      isPrem
                        ? "border-2 border-[#F8C927]/40 bg-white shadow-md"
                  ...
                  >
                    {isPrem && (
                      <div
                        ...
                        style={{
                          background: "linear-gradient(90deg, #F5A623, #F8C927, #F5A623)",
                        }}
                      />
                    )}
                    <div className="p-4">
                      <div className="flex items-center gap-3.5 mb-3">
                        <div
                          className={`w-14 h-14 rounded-xl overflow-hidden shrink-0 flex items-center justify-center ${
                            isPrem
                              ? "bg-[#F8C927]/10 border-2 border-[#F8C927]/30"
                          ...
                          {sponsorLogo ? (
                            ...
                            <span
                              className={`text-xl font-black ${
                                isPrem ? "text-[#F8C927]" : "text-gray-300"
                              }`}
                            >
                              {s.name.charAt(0)}
                            </span>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-[14px] font-extrabold text-[#0F1120] truncate">
                            {s.name}
                          </div>
                          <span
                            className={`inline-block mt-1 text-[8px] font-extrabold px-1.5 py-[2px] rounded-md uppercase tracking-wide ${
                              isPrem
                      ...
                      <div className="flex gap-2">
                        {sponsorWhatsapp && (
                          <Link
                            href={`https://wa.me/${sponsorWhatsapp}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-[#25D366] text-white text-[12px] font-bold active:scale-[0.97] transition-transform"
                          >
                        ...
                        {s.phone && (
                          <Link
                            href={`tel:${s.phone}`}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-[#0F1120] text-white text-[12px] font-bold active:scale-[0.97] transition-transform"
                          >
        ...
        <Link
          href="https://wa.me/5493535698990?text=Hola%2C%20quiero%20publicitar%20mi%20negocio%20en%20OficiosGo!"
          target="_blank"
          rel="noopener noreferrer"
        ...
        >
          <div
            ...
            style={{
              background: "linear-gradient(135deg, #0F1120 0%, #1A1D2E 50%, #252839 100%)",
            }}
          >
            <div
              className="absolute top-0 right-0 w-32 h-32 rounded-full"
              style={{
                background: "radial-gradient(circle, rgba(248,201,39,0.15), transparent 70%)",
                filter: "blur(20px)",
              }}
            />
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-lg bg-[#F8C927] flex items-center justify-center">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="#0F1120">
                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
                  </svg>
                </div>
                <span className="text-[10px] font-bold text-[#F8C927] uppercase tracking-widest">
                  Espacio publicitario
                </span>
              </div>
              <h3 className="text-[17px] font-black text-white leading-tight">
                ¡Que tu negocio aparezca en OficiosGo!
              </h3>
              <p className="text-[12px] text-gray-400 mt-2 leading-relaxed">
                Miles de vecinos de Villa María buscan servicios acá. Tu marca visible donde importa.
              </p>
              <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/10">
                ...
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#F8C927" strokeWidth="2.5" strokeLinecap="round">
                  <path d="m9 18 6-6-6-6" />
                </svg>
              </div>
            </div>
