...
export function ProfilePhotoUpload({ currentImage, userName }: { currentImage?: string | null; userName: string }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    const sx = (img.naturalWidth - size) / 2 - (pos.x / 280) * size;
    const sy = (img.naturalHeight - size) / 2 - (pos.y / 280) * size;
    ...
    try {
      ...
      if (!res.ok) alert("Error al subir la foto");
      ...
      {cropSrc && (
        <div className="fixed inset-0 z-[9999] bg-black/90 flex flex-col items-center justify-center p-4">
          ...
          <div className="w-full max-w-[340px]">
            ...
            <div className="relative w-[280px] h-[280px] mx-auto rounded-2xl overflow-hidden bg-black border-2 border-[#F8C927]/50" onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onEnd} onMouseLeave={onEnd} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onEnd}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img ref={imgRef} src={cropSrc} alt="Crop" className="absolute select-none pointer-events-none" draggable={false} style={{ width: `${100 * scale}%`, height: `${100 * scale}%`, objectFit: "cover", left: `${50 + pos.x / 2.8}%`, top: `${50 + pos.y / 2.8}%`, transform: "translate(-50%, -50%)" }} />
              <div className="absolute inset-0 border-2 border-white/30 rounded-2xl pointer-events-none" />
            </div>
            <div className="flex items-center gap-3 mt-3 px-2">
              <span className="text-white text-xs">-</span>
              <input type="range" min="1" max="3" step="0.05" value={scale} onChange={(e) => setScale(parseFloat(e.target.value))} className="flex-1 accent-[#F8C927]" />
              <span className="text-white text-xs">+</span>
            </div>
            <div className="flex gap-3 mt-4">
              <button onClick={() => setCropSrc(null)} className="flex-1 py-3 rounded-xl bg-white/10 text-white font-bold text-sm active:scale-[0.97] transition-transform">Cancelar</button>
              <button onClick={handleSave} className="flex-1 py-3 rounded-xl bg-[#F8C927] text-[#1A1D2E] font-extrabold text-sm active:scale-[0.97] transition-transform">Guardar foto</button>
            </div>
          </div>
        </div>
      )}
